...
public class ContainerReploidBuster extends Container
{
	/** The Item Inventory for this Container */
	public final InventoryBuster inventory;
	...
	private static final int INV_START = InventoryBuster.INV_SIZE, INV_END = INV_START+26,
			HOTBAR_START = INV_END+1, HOTBAR_END = HOTBAR_START+8;

	//If you're planning to add Buster slots, put those first like this:
	//Buster_START = InventoryItem.INV_SIZE, Buster_END = Buster_START+3,
	//INV_START = Buster_END+1, and then carry on like above.

	public ContainerReploidBuster(EntityPlayer par1Player, InventoryPlayer inventoryPlayer, InventoryBuster inventoryItem)
	{
		...
		for (i = 0; i < InventoryBuster.INV_SIZE; ++i)
		{
		...
		//If you want, you can add Buster SLOTS here as well, but you need to
		//make a public version of SlotBuster. I won't be doing that in this tutorial.
		/*
for (i = 0; i < 4; ++i)
{
//These are the standard positions for survival inventory layout
this.addSlotToContainer(new SlotBuster(this.player, inventoryPlayer, inventoryPlayer.getSizeInventory() - 1 - i, 8, 8 + i * 18, i));
}
		 */
	...
	public ItemStack transferStackInSlot(EntityPlayer par1EntityPlayer, int par2)
	{
		...
		if (slot != null && slot.getHasStack())
		{
			...
			//If item is in our custom Inventory or Buster slot
			if (par2 < INV_START)
			{
			...
			//Item is in inventory / hotbar, try to place in custom inventory or Buster slots
			else
			{
...
//you also add Buster or other custom slots
if (!this.mergeItemStack(itemstack1, 0, InventoryItem.INV_SIZE, false))
{
return null;
}
}
//If you added Buster slots, check them here as well:
//Item being shift-clicked is Buster - try to put in Buster slot
if (itemstack1.getItem() instanceof ItemBuster)
{
int type = ((ItemBuster) itemstack1.getItem()).BusterType;
if (!this.mergeItemStack(itemstack1, Buster_START + type, Buster_START + type + 1, false))
{
return null;
}
}
